...
Page({
  ...
  guideToPortPage: function (e) {
    var that = this;
    var ports = that.data.restaurant;
    wx.setStorageSync('current_port', ports[e.currentTarget.dataset.index]);
    wx.navigateTo({
      url: '../menu/menu',
    })
  },
              ...
              complete: res => {
                ...
                for (var i in temp.data) {
                  ...
                  for (var j in temp.data[i].foods) {
                    ...
                    singleFood.port_profile = temp.data[i].port_profile;
...
function addItemToWishList(item) {
  ...
  if (flag) {
    var newPort = {
      ...
      "port_profile": item.port_profile,
      foods: [
        item,
      ]
    };
...
function asjustOrderOfFoods(foods, food_selected) {
  ...
  if (food_selected == 4) {
    ...
    for (var i = 1; i < len; i++) {
      var temp = foods[i];
      var j = i - 1;//默认已排序的元素
      while (j >= 0 && (foods[j].like * 1.0 / foods[j].total_people) < (temp.like * 1.0 / temp.total_people)) {  //在已排序好的队列中从后向前扫描
        foods[j + 1] = foods[j]; //已排序的元素大于新元素，将该元素移到一下个位置
        j--;
      }
      foods[j + 1] = temp;
